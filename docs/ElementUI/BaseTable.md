...
<script>
  export default {
    data() {
      ...
      const addressMap = [
        { label: '上海市普陀区金沙江路 1519 弄', value: '1519' },
        { label: '上海市普陀区金沙江路 1518 弄', value: '1518' },
        { label: '上海市普陀区金沙江路 1517 弄', value: '1517' },
        { label: '上海市普陀区金沙江路 1516 弄', value: '1516' }
      ]
      return {
        data: [
          { name: '王小虎', age: 19, money: 19.99, address: '1519' },
          { name: '王小虎', age: 18, money: 18.99, address: '1518' },
          { name: '王小虎', age: 17, money: 17.99, address: '1517' },
          { name: '王小虎', age: 16, money: 16.99, address: '1516' }
        ],
        columns: [
          { label: '名字', prop: 'name' },
          { label: '年龄', prop: 'age' },
  ...
  export default {
    data() {
      ...
      return {
        ...
        columns: [
          { label: '名字', prop: 'name', editable: true, component: 'el-input', type: 'text' },
          {
            label: '年龄',
            prop: 'age',
            editable: true,
          ...
          { label: '资产', prop: 'money', editable: true, component: 'number-input' },
          {
            label: '住址',
            prop: 'address',
            width: '300px',
...
:::demo 通过 `focusRow` 和 `focusCol` 控制聚焦行列
...
:::demo 通过传入 `currentNodeKey` 和 `defaultCheckedKeys` 设置默认选中, 必须传入 `rowKey`, 默认为 `id`. 此例同时展示了通过 `slot` 来传入列, 默认在 `columns` 后面, 通过具名 `prev` 可以设置到 `columns` 前面
...
**注: 由于 `type` 在 `EditableElements` 中被占用, 所以 `Table-column` 中的 `type` 类型行只能通过 `slot` 传入, 而不能通过 `columns` 传入, 详见[默认选中](/ElementUI/BaseTable.html#默认选中)**
